const fetchFakeData = async centerCoordinates => {
  ...
  let points = [];
  let results = await getDataFromURL(getURL(1000, centerCoordinates.latitude, centerCoordinates.longitude, 100));

  for (let result of results)
  {
      if(results.some(x => x.id != result.id && x.wikidata === result.wikidata))
        continue;
